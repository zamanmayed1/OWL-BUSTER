...
const extractValidEmails = (text) => {
  ...
  const isBusinessEmail = (email) => {
    ...
    return !exampleDomains.includes(emailDomain) && !emailDomain.match(/\.(png|jpeg|jpg|gif|webp)$/);
  };

  return emails.filter((email) => validator.validate(email) && isBusinessEmail(email)).slice(0, 3); // Limit to 3 emails
};
...
const crawlWebsiteForEmails = async (siteUrl) => {
  ...
  const contactPaths = [
    '/contact', '/contact-us', '/contact-us.html', '/contact-us.php', '/contact-us.aspx',
    '/contact.html', '/contact.php', '/contact.aspx', '/en/contact-us', '/en/contact',
    '/fr/contact', '/es/contacto', '/de/kontakt', '/contact-us-de', '/support/contact-us',
    '/get-in-touch', '/reach-out', '/contact-info', '/contact-information', '/contact_us',
